...
class SharedReplayBuffer(object):
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
        ...
        if isinstance(share_obs, dict):
            # 将字典转换为numpy数组
            share_obs_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.share_obs.shape[3:]))
            for i, agent_id in enumerate(share_obs.keys()):
                if isinstance(share_obs[agent_id], (np.ndarray, list)):
                    share_obs_array[0, i] = np.array(share_obs[agent_id], dtype=np.float32)
                else:
                    share_obs_array[0, i] = np.array([float(share_obs[agent_id])], dtype=np.float32)
            share_obs = share_obs_array

        if isinstance(obs, dict):
            # 将字典转换为numpy数组
            obs_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.obs.shape[3:]))
            for i, agent_id in enumerate(obs.keys()):
                if isinstance(obs[agent_id], (np.ndarray, list)):
                    obs_array[0, i] = np.array(obs[agent_id], dtype=np.float32)
                else:
                    obs_array[0, i] = np.array([float(obs[agent_id])], dtype=np.float32)
            obs = obs_array

        # 处理字典类型的动作
        if isinstance(actions, dict):
            actions_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.actions.shape[2:]))
            for i, agent_id in enumerate(actions.keys()):
                if isinstance(actions[agent_id], (np.ndarray, list)):
                    actions_array[0, i] = np.array(actions[agent_id], dtype=np.float32)
                else:
                    actions_array[0, i] = np.array([float(actions[agent_id])], dtype=np.float32)
            actions = actions_array

        # 处理字典类型的动作对数概率
        if isinstance(action_log_probs, dict):
            action_log_probs_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.action_log_probs.shape[2:]))
            for i, agent_id in enumerate(action_log_probs.keys()):
                if isinstance(action_log_probs[agent_id], (np.ndarray, list)):
                    action_log_probs_array[0, i] = np.array(action_log_probs[agent_id], dtype=np.float32)
                else:
                    action_log_probs_array[0, i] = np.array([float(action_log_probs[agent_id])], dtype=np.float32)
            action_log_probs = action_log_probs_array

        # 处理字典类型的值函数预测
        if isinstance(value_preds, dict):
            value_preds_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.value_preds.shape[2:]))
            for i, agent_id in enumerate(value_preds.keys()):
                if isinstance(value_preds[agent_id], (np.ndarray, list)):
                    value_preds_array[0, i] = np.array(value_preds[agent_id], dtype=np.float32)
                else:
                    value_preds_array[0, i] = np.array([float(value_preds[agent_id])], dtype=np.float32)
            value_preds = value_preds_array

        # 处理字典类型的奖励
        if isinstance(rewards, dict):
            rewards_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.rewards.shape[2:]))
            for i, agent_id in enumerate(rewards.keys()):
                if isinstance(rewards[agent_id], (np.ndarray, list)):
                    rewards_array[0, i] = np.array(rewards[agent_id], dtype=np.float32)
                else:
                    rewards_array[0, i] = np.array([float(rewards[agent_id])], dtype=np.float32)
            rewards = rewards_array

        # 处理字典类型的掩码
        if isinstance(masks, dict):
            masks_array = np.zeros((self.n_rollout_threads, self.num_agents, *self.masks.shape[2:]))
            for i, agent_id in enumerate(masks.keys()):
                if isinstance(masks[agent_id], (np.ndarray, list)):
                    masks_array[0, i] = np.array(masks[agent_id], dtype=np.float32)
                else:
                    masks_array[0, i] = np.array([float(masks[agent_id])], dtype=np.float32)
            masks = masks_array

        def _safe_to_numpy(tensor_or_array, dtype=np.float32):
            """将tensor或数组转换为numpy数组"""
            if isinstance(tensor_or_array, torch.Tensor):
                if tensor_or_array.is_cuda:
                    return tensor_or_array.cpu().detach().numpy().astype(dtype)
                else:
                    return tensor_or_array.detach().numpy().astype(dtype)
        ...
        # 确保所有输入都是numpy数组
        share_obs = _safe_to_numpy(share_obs)
        obs = _safe_to_numpy(obs)
        rnn_states_actor = _safe_to_numpy(rnn_states_actor)
        rnn_states_critic = _safe_to_numpy(rnn_states_critic)
        actions = _safe_to_numpy(actions)
        action_log_probs = _safe_to_numpy(action_log_probs)
        value_preds = _safe_to_numpy(value_preds)
        rewards = _safe_to_numpy(rewards)
        masks = _safe_to_numpy(masks)
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        next_value = _safe_to_numpy(next_value)
